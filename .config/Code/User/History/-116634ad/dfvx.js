...
function clockwork(){       //instruction for clockwork is:
    ...
    // console.log(t);          // 3nd display t using console.log to check

    const hour = t.getHours()   // 2nd grab t (or date/time) and get actual Hours only, 
                                // then let it be hour

    const minute = t.getMinutes() // 2nd grab t (or date/time) and get actual Minutes only, 
                                  // then let it be minute

    const second = t.getSeconds() // 2nd grab t (or date/time) and get actual Seconds only, 
                                  // then let it be second

    // console.log(hour, minute, second); // used console.log to check variables 

    const hourDeg = (hour/12) * 360;   // 3rd declare variable hourDeg value and converted 
                                       // actual hour to a degrees value 

    hrEl.style.transform = `rotate(${hourDeg}deg)`; // 4th similar to css we
    //want hrEl piece to shape and rotate at wherever and whenevr degree-hour get updates

    const minuteDeg = (minute/60) * 360;
    minEl.style.transform = `rotate(${minuteDeg}deg)`; //4th 

    const secondDeg = (second/60) * 360;
    secEl.style.transform = `rotate(${secondDeg}deg)`; //4th
}   

setInterval(clockwork, 1000) // a call to START clockwork function with a repeat of 1s /1000milsec